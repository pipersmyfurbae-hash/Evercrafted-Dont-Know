 ...
 * call sites now share one implementation, so there is nothing left to diff.
 */
...
/** Public match shape returned across the API boundary — omits score, per the migration plan's public response contract. */
export interface PublicMoodoorMatch {
  listing: Omit<MoodoorListing, 'quality' | 'isMoodoorPublished' | 'publishedAt'>;
  explanation: string;
  matchedSignals: string[];
}
...
export const MINIMUM_QUALITY_SCORE = 0.78;
export const MINIMUM_MATCH_SCORE = 0.42;
export const MAX_MATCHES = 3;

export const moodSignals: Record<MoodId, string[]> = {
...
export const doorSignals: Record<DoorId, string[]> = {
...
export function resolveQuality(raw: MarketplaceDocument): MoodoorListing['quality'] {
...
/** Strips internal fields before a match crosses the public API boundary. */
export function toPublicMatch(match: MoodoorMatch): PublicMoodoorMatch {
  const { quality: _quality, isMoodoorPublished: _isMoodoorPublished, publishedAt: _publishedAt, ...publicListing } = match.listing;
  return {
...
export function isMoodId(value: unknown): value is MoodId {
  return typeof value === 'string' && value in moodSignals;
}

export function isSeasonId(value: unknown): value is SeasonId {
  return typeof value === 'string' && seasonChoices.some((choice) => choice.id === value);
}

export function isDoorId(value: unknown): value is DoorId {
  return typeof value === 'string' && value in doorSignals;
}
